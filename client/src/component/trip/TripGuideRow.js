...
class AddGuideRow extends PureComponent {
  ...
  templateNormalizer = (value, previousValue, allValues) => {
    const { startTime, endTime, guides, cost, kidsUnder12 } = allValues
    const {guests, requested } = guides[ this.props.index ]
    if (value) {
      return value
    } else {
      return `${startTime ? this.formatDate(startTime) : ''}, ${startTime ? this.formatHours(startTime) : ''} - ${endTime ? this.formatHours(endTime) : ''} for ${guests || ''} people ${kidsUnder12 ? `with ${kidsUnder12} kids under 12`: ''}. Cost $${cost || ''} ${requested ? '- Requested' : ''}`
    }
  }
  ...
  render() {
    return (
      <div className="TripGuideRow" key={`Guide-${this.props.index + 1}`}>
        ...
        />
        <Field name={`${this.props.field}.requested`}
               className="guide-field"
               component={Checkbox}
