...
class AddTrip extends Component {
  ...
  getTripValues = (values) => {
    return {
      ...values,
      startTime: moment(values.startTime).tz('America/Denver').toISOString(),
      endTime: moment(values.endTime).tz('America/Denver').toISOString(),
      userName: this.props.user.name.split(' ')[ 0 ]
    }
  }
    ...
    return (
      <div className="guides-section">
        <h3 className="group-header">
          Guides
          <button type="button" className="btn btn-primary add-guide"
                  onClick={() => fields.unshift({ sendConfirmation: true, guests: this.props.guests })}>
            + Add Guide
          </button>
        </h3>
        <div>
        ...
        <form className="panel panel-primary" onSubmit={handleSubmit(this.handleSubmit)}>
          ...
          <div className="panel-body">
            <div className="form-fields">
              ...
              <Field name="staying"
                     component={TextField}
                     label="Do you know where will you be staying?"
